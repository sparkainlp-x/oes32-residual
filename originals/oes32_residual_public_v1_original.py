...
def run_monte_carlo(n_runs=N_RUNS):
    ...
    for i in range(n_runs):
        params = {
            "J":     np.random.uniform(1.9, 2.6),
            "lam":   np.random.uniform(2.4, 3.2),
            "gamma": np.random.uniform(1.5, 2.1),
            "delta": np.random.uniform(0.95, 1.35),
            "eta":   np.random.uniform(0.14, 0.24),
            "noise": np.random.uniform(0.05, 0.12),
            "level": np.random.randint(2, 5),
            "Psi0":  np.random.uniform(0.40, 0.70),
...
D   = np.array([r["D"] for r in mc])
L   = np.array([r["loop"] for r in mc])

print("=== Public Residual Results ===")
print(f"Mean S(R)          : {np.mean(S_R):.4f} ± {np.std(S_R):.4f}")
print(f"Median S(R)        : {np.median(S_R):.4f}")
print(f"5% – 95% quantiles : [{np.quantile(S_R, 0.05):.4f}, {np.quantile(S_R, 0.95):.4f}]")
print(f"Fraction S(R) > 0  : {np.mean(S_R > 0)*100:.1f}%")
print(f"Loop entry rate    : {np.mean(L)*100:.1f}%")
print()
print("Residual definition used (exact):")
print("  S(R) = max(0, 0.55 * D + 0.30 * (1 - L))")
print()
print("This is the quantity that must be able to die.")